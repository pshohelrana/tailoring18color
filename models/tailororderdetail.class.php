...
class TailorOrderDetail implements JsonSerializable{
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select od.id,d.name dresses,od.price,od.qty,od.discount,o.id from core_tailor_order_details od,core_tailor_dresses d,core_tailor_orders o where d.id=od.dress_id and o.id=od.order_id");
	...
	public static function all_by_order_id($order_id){
		global $db,$tx;
		$result=$db->query("select od.id,p.name ,od.price,od.qty,od.discount from core_tailor_order_details od,core_tailor_dresses p where p.id=od.dress_id and order_id={$order_id} ");
		$data=[];
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		$top = ($page - 1)*$perpage;
		$result=$db->query("select o.id,d.name tailor_dresses,o.price,o.qty,o.discount,t.customer_name tailor_orders from {$tx}tailor_dresses d,{$tx}tailor_orders t,{$tx}tailor_order_details o where d.id=o.dress_id and t.id=o.order_id $criteria limit $top,$perpage");
		$html="<table class='table'>";
			$html.="<tr><th colspan='3'><a class=\"btn btn-success\" href=\"create-tailororderdetail\">New TailorOrderDetail</a></th></tr>";
		if($action){
			$html.="<tr><th>Id</th><th>Dresses name</th><th>Price</th><th>Qty</th><th>Discount</th><th>Customer</th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Dresses name</th><th>Price</th><th>Qty</th><th>Discount</th><th>Customer</th></tr>";
		}
		while($tailororderdetail=$result->fetch_object()){
			$action_buttons = "";
			if($action){
				$action_buttons = "<td><div class='clearfix' style='display:flex;'>";
				$action_buttons.= action_button(["id"=>$tailororderdetail->id, "name"=>"Details", "value"=>"Details", "class"=>"dark", "url"=>"details-tailororderdetail"]);
				// $action_buttons.= action_button(["id"=>$tailororderdetail->id, "name"=>"Edit", "value"=>"Edit", "class"=>"primary", "url"=>"edit-tailororderdetail"]);
				$action_buttons.= action_button(["id"=>$tailororderdetail->id, "name"=>"Delete", "value"=>"Delete", "class"=>"dark", "url"=>"tailor_order_details"]);
				$action_buttons.= "</div></td>";
			}
			$html.="<tr><td>$tailororderdetail->id</td><td>$tailororderdetail->tailor_dresses</td><td>$tailororderdetail->price</td><td>$tailororderdetail->qty</td><td>$tailororderdetail->discount</td><td>$tailororderdetail->tailor_orders</td> $action_buttons</tr>";
		}
		$html.="</table>";
